...
// Database connection
const mongoUri =
  process.env.MONGODB_URI || 'mongodb://localhost:27017/seatmgmt';
...
mongoose
  ...
  .then((collections) => {
    ...
    // timeslots 컬렉션 생성 확인
    const hasTimeslotsCollection = collections.some(
      (c) => c.name === 'timeslots'
    );
    if (!hasTimeslotsCollection) {
      logger.info(
        'timeslots 컬렉션이 없습니다. 첫 번째 일정이 추가되면 자동 생성될 것입니다.'
      );
    } else {
        ...
        .then((count) => {
          ...
          if (count > 0) {
            return mongoose.connection.db
              ...
              .toArray()
              .then((samples) => {
                logger.debug('TimeSlots 샘플 조회 완료', {
  ...
  app.get('/api/debug/stats', async (req, res) => {
    try {
      const usersCount = await mongoose.connection.db
        .collection('users')
        .countDocuments();
      const seatsCount = await mongoose.connection.db
        .collection('seats')
        .countDocuments();
      const timeslotsCount = await mongoose.connection.db
        .collection('timeslots')
        .countDocuments();
