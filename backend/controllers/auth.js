...
exports.login = async (req, res) => {
  try {
    ...
    if (!user) {
      logger.logAuth('login', studentId, false, { reason: 'user_not_found', name });
    ...
    if (password) {
      ...
      if (!user.password) {
        ...
        logger.logAuth('login', studentId, false, { reason: 'no_password_set', name });
        return res.status(401).json({
          success: false,
      ...
      if (!isMatch) {
        logger.logAuth('login', studentId, false, { reason: 'invalid_password', name });
        return res.status(401).json({
          success: false,
          message: 'Invalid credentials'
        });
      ...
      if (user.password) {
        ...
        logger.logAuth('login', studentId, false, { reason: 'password_required', name });
        return res.status(401).json({
          success: false,
...
exports.changePassword = async (req, res) => {
  try {
    ...
    if (!user) {
      logger.warn('User not found in changePassword', { studentId: req.user.studentId });
      return res.status(404).json({
        success: false,
        message: 'User not found'
      });
    ...
    if (user.password) {
      ...
      if (!isMatch) {
        logger.logAuth('change_password', req.user.studentId, false, { reason: 'invalid_current_password' });
        return res.status(401).json({
          success: false,
